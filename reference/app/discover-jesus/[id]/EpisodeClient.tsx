...
export default function EpisodeClient({ params }: Props) {
  ...
  const prevEpisode = getDiscoverJesusEpisodes().find(ep => ep.id === episodeId - 1);
  const nextEpisode = getDiscoverJesusEpisodes().find(ep => ep.id === episodeId + 1);
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="relative group cursor-pointer rounded-2xl overflow-hidden shadow-2xl">
              <Image
                src={episode.imageUrl || '/images/default-profile.jpg'}
                alt="Jesus Profile"
                width={600}
                height={600}
            ...
            <div className="flex justify-between gap-4">
              {prevEpisode && (
                <Link 
                  href={`/discover-jesus/${prevEpisode.id}`}
              ...
              {nextEpisode && (
                <Link 
                  href={`/discover-jesus/${nextEpisode.id}`}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
                >
                  Next
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <a 
                  href="https://discoverjesus.com/events"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block text-primary hover:text-primary-dark transition-colors font-fira-sans"
                >
