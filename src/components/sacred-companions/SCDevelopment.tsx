...
interface StageCardProps {
  ...
  index: number;
  isLast: boolean;
  subtitle?: string; // Optional subtitle for flexibility
}

function StageCard({ title, description, points, index, isLast, subtitle }: StageCardProps) {
  // Guard against missing data
  if (!points?.length || !title || !description) {
    console.warn('StageCard: Missing required props');
    return null;
  }
...
export function SCDevelopment() {
  const { development } = scContent;
  
  // Guard against missing content
  if (!development?.sections?.length) {
    console.warn('SCDevelopment: Missing required content');
    return null;
  }
